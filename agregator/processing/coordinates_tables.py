...
from agregator.processing.geo_utils import calculate_polygons_area, dms_to_decimal, convert_to_wgs84, COORDINATE_MARKS, \
    COORDINATE_SYSTEMS
...
def search_coords_in_text(pdf, page_number, document, tables, text, coordinates):
    ...
    pits_coordinates = re.findall(
        ...
        text, re.IGNORECASE)
    pits_coordinates.extend(re.findall(
        r'Шурф\s[№\s]*\d+[\s\S]+?[\s\S]+?[NS]\s*\d+°\d+\'\d+[\.,]*\d+",*\s+[EW]\s*\d+°\d+\'\d+[\.,]*\d+"',
        text, re.IGNORECASE))
    # Шурф\s+№\s+\d+\**[\s\S]+?Координаты\s+шурфа\s+в\s+системе\s+WGS-\d+:*\s+[NSEW]\s*?\d+°\d+\'\d+[\.,]\d+";*\s+[NSEW]\s*?\d+°\d+\'\d+[\.,]\d+"
