...
FULL_TIME_INTERVAL_PATTERN_VAR_3 = re.compile(
    r'^(?!.*действителен\b).*?[сc]\s+([0-3]?\s*[0-9]\s*[\. ]+\s*[0-1]?[0-9]\s*[\. ]+\s*\d{2,4})\s+[г.\s]*[-–—по]+\s+([0-3]?\s*[0-9]\s*[\. ]+\s*[0-1]?[0-9]\s*[\. ]+\s*\d{2,4})\s*[г\.]*',
    re.IGNORECASE)  # r'период с «*\d+»* [А-Яа-яёЁ]+ \d+ г\.*.*\s+по\s+(«*\d+»*\s*[А-Яа-яёЁ]+\s*\d+)[\sг\.]*'
...
FULL_TIME_INTERVAL_PATTERN_VAR_6 = re.compile(
    r'^(?!.*действителен\b).*?([0-3]?\s*[0-9]\s*[\. ]+\s*[0-1]?[0-9]\s*[\. ]+\s*\d*|«*\d+»*\s*[А-Яа-яёЁ]+\s*\d*)\s*[\sгода\.]*.{0,5}?[-–—по]+\s+([0-3]?\s*[0-9]\s*[\. ]+\s*[0-1]?[0-9]\s*[\. ]+\s*\d{2,4}|«*\d+»*\s*[А-Яа-яёЁ]+\s*\d+)\s*[\sгода\.]*',
    re.IGNORECASE | re.MULTILINE)  # Дата\s*начала\s*и\s*
...
RE_OBJECT_ZEMLI = re.compile(
    r'Объект\s*(государственной\s*историко[-–—\s]*культурной\s*)?(экспертизы)?(?!.*(?:наслед))[\s\S]{0,15}земли',
    re.IGNORECASE)
RE_OBJECT_ZEMELNY = re.compile(
    r'Объект\s*(государственной\s*историко[-–—\s]*культурной\s*)?(экспертизы)?(?!.*(?:наслед))[\s\S]{0,15}земельны',
    re.IGNORECASE)
RE_OBJECT_RAZDEL = re.compile(
    r'Объект\s*(государственной\s*историко[-–—\s]*культурной\s*)?(экспертизы)?(?!.*(?:наслед))[\s\S]{0,15}раздел',
    re.IGNORECASE)
RE_OBJECT_DOC = re.compile(
    r'Объект\s*(государственной\s*историко[-–—\s]*культурной\s*)?(экспертизы)?(?!.*(?:наслед))[\s\S]{0,15}документация',
    re.IGNORECASE)
