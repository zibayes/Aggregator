...
def process_scientific_reports(self, reports_ids, user_id, select_text, select_enrich, select_image, select_coord,
                               is_reprocess=False):
    document_type = 'scientific_reports'
    ...
    try:
        progress_json = process_documents(self, reports_ids, user_id, document_type,
...
def extract_text_and_images(current_report, file, progress_recorder, pages_count, total_processed, progress_json,
    ...
    if is_reprocess is False:
        has_duplicates, duplicate_id = has_duplicates_in_db(ScientificReport, file, report_id)
        if has_duplicates:
            raise FileExistsError(
                f"Такой файл уже загружен в систему (report.id={duplicate_id}): {progress_json['file_groups'][str(report_id)][source_index]['origin_filename']}")
