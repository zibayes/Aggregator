...
def generate_download_report(all_files_info: List[Dict], report_path: str = None, additional_header: str = ""):
    ...
    logger.info(f"🚀 НАЧАЛО ГЕНЕРАЦИИ ОТЧЕТА. Файлов: {len(all_files_info)}")
    ...
    try:
        ...
        logger.info(f"📁 Директория создана/проверена: {os.path.dirname(report_path)}")
        ...
        logger.info(
            f"📊 Статистика: всего={total_files}, скачано={downloaded_count}, пропущено={skipped_count}, в очереди={queued_count}, ошибок={error_count}")
...
def generate_interrupted_report(current_report_data):
    ...
    logger.info("🔄 ГЕНЕРАЦИЯ ОТЧЕТА О ПРЕРЫВАНИИ")

    if not current_report_data['files_info']:
        logger.warning("📭 Нет данных для отчета о прерывании")
    ...
    try:
        ...
        <div class="warning-banner">
            ...
            Время прерывания: {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}<br>
            {'Диапазон дат: ' + str(current_report_data['start_date']) + ' - ' + str(current_report_data['end_date']) + '<br>' if current_report_data.get('start_date') and current_report_data.get('end_date') else ''}
            {'Диапазон страниц: ' + str(current_report_data['start_page']) + ' - ' + str(current_report_data['end_page']) + '<br>' if current_report_data.get('start_page') and current_report_data.get('end_page') else ''}
        </div>
        """
        ...
        logger.info(f"✅ Отчет о прерванной задаче сохранен: {report_path}")
        ...
        logger.info(f"💾 Бэкап данных сохранен: {json_path}")

    except Exception as e:
        logger.error(f"❌ Ошибка при генерации отчета о прерывании: {e}")
...
def generate_final_report(current_report_data):
    ...
    try:
        ...
        <div class="success-banner">
            ...
            Обработано страниц: {current_report_data['processed_pages']} из {current_report_data['total_pages']}<br>
            Время начала: {current_report_data['start_time']}<br>
            Время завершения: {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}<br>
            {'Диапазон дат: ' + str(current_report_data['start_date']) + ' - ' + str(current_report_data['end_date']) + '<br>' if current_report_data.get('start_date') and current_report_data.get('end_date') else ''}
            {'Диапазон страниц: ' + str(current_report_data['start_page']) + ' - ' + str(current_report_data['end_page']) + '<br>' if current_report_data.get('start_page') and current_report_data.get('end_page') else ''}
        </div>
        """
        ...
        logger.error(f"❌ Ошибка при генерации финального отчета: {e}")
...
def generate_intermediate_report(current_report_data):
    ...
    logger.info("🔄 ГЕНЕРАЦИЯ ПРОМЕЖУТОЧНОГО ОТЧЕТА")
    logger.info(
        f"📊 Данные для отчета: start_time={current_report_data.get('start_time')}, processed_pages={current_report_data.get('processed_pages')}, total_pages={current_report_data.get('total_pages')}")

    try:
        report_path = 'uploaded_files/Акты ГИКЭ/intermediate_report.html'

        # Форматируем даты правильно
        start_date_str = str(current_report_data.get('start_date', '')) if current_report_data.get('start_date') else ''
        end_date_str = str(current_report_data.get('end_date', '')) if current_report_data.get('end_date') else ''
        start_page_str = str(current_report_data.get('start_page', '')) if current_report_data.get('start_page') else ''
        end_page_str = str(current_report_data.get('end_page', '')) if current_report_data.get('end_page') else ''

        intermediate_note = f"""
        <div class="info-banner">
            🔄 <strong>Промежуточный отчет</strong><br>
            Обработано страниц: {current_report_data.get('processed_pages', 0)} из {current_report_data.get('total_pages', 0)}<br>
            Время начала: {current_report_data.get('start_time', 'Неизвестно')}<br>
            Последнее обновление: {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}<br>
            {f'Диапазон дат: {start_date_str} - {end_date_str}<br>' if start_date_str and end_date_str else ''}
            {f'Диапазон страниц: {start_page_str} - {end_page_str}<br>' if start_page_str and end_page_str else ''}
        </div>
        """
        ...
        logger.info(f"✅ Промежуточный отчет сохранен: {report_path}")
        print(f"Промежуточный отчет сохранен: {report_path}")

    except Exception as e:
        logger.error(f"❌ Ошибка при генерации промежуточного отчета: {e}")
...
class TaskState:
    ...
    def update(self, **kwargs):
        logger.info(f"🔄 Обновление состояния задачи: {kwargs}")
...
def setup_interrupt_handling(task_state):
    ...
    def signal_handler(signum, frame):
        logger.info(f"🛑 Получен сигнал {signum}, генерируем отчет...")
        print(f"Получен сигнал {signum}, генерируем отчет...")
        generate_interrupted_report(task_state.get_data())
        sys.exit(1)

    def exit_handler():
        logger.info("🛑 Задача завершается, генерируем отчет...")
...
def handle_interrupts(func):
    ...
    def wrapper(self, *args, **kwargs):
        logger.info("🎬 ЗАПУСК ЗАДАЧИ С ОБРАБОТКОЙ ПРЕРЫВАНИЙ")

        # Создаем состояние задачи ВНУТРИ обёртки
        task_state = TaskState()
        current_time = datetime.now().strftime('%d.%m.%Y %H:%M:%S')
        task_state.update(start_time=current_time)

        logger.info(f"🕐 УСТАНОВЛЕНО ВРЕМЯ ЗАПУСКА: {current_time}")

        # Настраиваем обработчики прерываний
        setup_interrupt_handling(task_state)

        try:
            logger.info("▶️ ВЫЗОВ ОСНОВНОЙ ФУНКЦИИ")
            # Передаем состояние задачи в основную функцию как первый аргумент
            result = func(self, task_state, *args, **kwargs)
            logger.info("✅ ОСНОВНАЯ ФУНКЦИЯ УСПЕШНО ЗАВЕРШЕНА")
            return result
        except Exception as e:
